...
$(function() {
	...
	inintUPdate();
	showTestList(true);
	sessionStorage.setItem('position','考试')
...
function showTestList(first, pageIndex){
...
function LookClass(datalist, num) {
	if(datalist&&datalist.length > 0) {
		...
		for(i = 0; i < datalist.length; i++) {
			...
			var al = "";
			if(datalist[i].testStatus === "NOTSTART") {
				al = "未发布";
			} else if(datalist[i].testStatus === "FINISH") {
				al = "已停止";
			} else if(datalist[i].testStatus === "PROGRESS") {
				al = "已发布";
			} else if(datalist[i].testStatus === "COMPLETED") {
				al = "已结束";
			}
			var dateStr = '-';	
			...
			con.innerHTML += '<div class="room-static"><label>考试名称：</label><span name="name">' + datalist[i].testName + '</span></div>';
			con.innerHTML += '<div class="room-class-two"><label>对应班级：</label><span name="className">' + datalist[i].className + '</span></div>';
			con.innerHTML += '<div class="room-introduction  btn btn8" style="left:106px;top:84px;background-color:#59e8e3;" onclick="showInfo(this)">考试范畴<input type="hidden" name="info" value="' + datalist[i].testInfo + '"/></div>';	
			if(datalist[i].testStatus === "NOTSTART") {			
				con.innerHTML += '<div class="class-but-ph-hk" onclick="updateTestStatus(\'PROGRESS\',this)"><input type="hidden"  value="' + id + '"/>发布考试</div>';
			}else if(datalist[i].testStatus === "PROGRESS"){
				con.innerHTML += '<div class="class-but-ph-hk" onclick="updateTestStatus(\'FINISH\',this)"><input type="hidden"  value="' + id + '"/>终止作答</div>';
			}else if(datalist[i].testStatus === "COMPLETED"){
				// con.innerHTML += '<div class="class-but-ph-hk" onclick="updateTestStatus(\'PROGRESS\',this)"><input type="hidden"  value="' + id + '"/>重新发布</div>';
				con.innerHTML += '<div class="class-but-ph-hk" style="background-color:#999"><input type="hidden"  value="' + id + '"/>考试已结束</div>';
			}
			else if(datalist[i].testStatus === "FINISH"){
				con.innerHTML += '<div class="class-but-ph-hk" onclick="updateTestStatus(\'PROGRESS\',this)"><input type="hidden"  value="' + id + '"/>重新发布</div>';
			}
			...
			if(datalist[i].testStatus === "NOTSTART") {
				con.innerHTML += '<div class="but-update" style="left:-18px"  onclick="ShowDiv(MyDiv,fade,this)"><input type="hidden" name="id" value="' + id + '"  />修改考试</div>';
			}
			if(datalist[i].testStatus === "NOTSTART") {
			con.innerHTML += '<div class="class-but-del-hk" onclick="DelDate(this)"><input type="hidden"  value="' + id + '"  />删除考试</div>';
			}
			if(datalist[i].testStatus === "FINISH") {
				con.innerHTML += '<div class="class-but-del-hk" style="background-color:#3D72FE" onclick="updateTestStatus(\'COMPLETED\',this)"><input type="hidden"  value="' + id + '"  />结束考试</div>';
				}
...
function updateTestStatus(newStatus, obj){
	...
	$.ajax({
		url: local + "/TESTSERVICE/console/updateTestStatus",
		headers: {
			'accessToken': accessToken
		},
		type: 'POST',
...
function updateSub() {
	...
	$.ajax({
		url: local + "/TESTSERVICE/console/updateTestContent",
		headers: {
			'accessToken': accessToken
		},
		type: 'POST',
